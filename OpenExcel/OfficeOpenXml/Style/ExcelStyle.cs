...
namespace OpenExcel.OfficeOpenXml.Style
{
    public class ExcelStyle
    {
        ...
        public ExcelNumberFormat NumberFormat
        {
            ...
            set
            {
                this.StyleIndex = _styles.MergeAndRegisterCellFormat(new CellFormat() { NumberFormatId = value.NumFmtId, ApplyNumberFormat = true }, this.StyleIndex);
                if (_stylable != null)
        ...
        public ExcelFont Font
        {
            ...
            set
            {
                CellFormat cf = _styles.GetCellFormat(this.StyleIndex ?? 0);
                uint fontId = cf.FontId ?? 0;
                uint newFontId = _styles.MergeAndRegisterFont(value.FontObject, fontId);
                if (newFontId != fontId)
                {
                    this.StyleIndex = _styles.MergeAndRegisterCellFormat(new CellFormat() { FontId = newFontId, ApplyFont = true }, this.StyleIndex);
                    if (_stylable != null)
                        _stylable.Style = this;
                }
            }
        ...
        public ExcelBorder Border
        {
            ...
            set
            {
                CellFormat cf = _styles.GetCellFormat(this.StyleIndex ?? 0);
                uint borderId = cf.BorderId ?? 0;
                uint newBorderId = _styles.MergeAndRegisterBorder(value.BorderObject, borderId);
                if (newBorderId != borderId)
                {
                    this.StyleIndex = _styles.MergeAndRegisterCellFormat(new CellFormat() { BorderId = newBorderId, ApplyBorder = true }, this.StyleIndex);
                    if (_stylable != null)
                        _stylable.Style = this;
                }
            }
        ...
        public ExcelFill Fill
        {
            ...
            set
            {
                CellFormat cf = _styles.GetCellFormat(this.StyleIndex ?? 0);
                uint fillId = cf.FillId ?? 0;
                uint newFillId = _styles.MergeAndRegisterFill(value.FillObject, fillId);
                if (newFillId != fillId)
                {
                    this.StyleIndex = _styles.MergeAndRegisterCellFormat(new CellFormat() { FillId = newFillId, ApplyFill = true }, this.StyleIndex);
                    if (_stylable != null)
                        _stylable.Style = this;
                }
            }
