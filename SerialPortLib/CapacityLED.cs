...
namespace SerialPortLib
{
	...
	public class CapacityLED
	{
		#region import
		[System.Runtime.InteropServices.DllImport("CapacityLib.dll")]
		...
		public bool SendCapacityToLedBoard(int CapacityValue)
		{
			if (Constants.SoftwareMode == Mode.Use && _isInitialized)
			{
				/*unsafe
				{
					try
					{
						byte[] command = Encoding.ASCII.GetBytes("Z");
						byte[] command2 = Encoding.ASCII.GetBytes(CapacityValue.ToString());
					...
					{
						_isInitialized = false;
						_errors = "خطا در ارسال پیام به تابلو ظرفیت.";
						return false;
					}
				}*/
				try
				{
					byte[] command = Encoding.ASCII.GetBytes("CAP;" + CapacityValue + ";");
