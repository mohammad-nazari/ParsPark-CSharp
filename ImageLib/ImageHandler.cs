...
namespace ImageLib
{
	public class ImageHandler
	{
		...
		public void SetColorFilter(Bitmap ImageObject, Color InputColor)
		{
			Bitmap bmap = (Bitmap)ImageObject.Clone();
			Color c;
			for(int i = 0; i < bmap.Width; i++)
			{
				for(int j = 0; j < bmap.Height; j++)
				{
					...
					nPixelR = c.R + InputColor.R;
					nPixelG = c.G + InputColor.G;
					nPixelB = c.B + InputColor.B;
					...
					bmap.SetPixel(i, j, Color.FromArgb(c.A, (byte)nPixelR, (byte)nPixelG, (byte)nPixelB));
				}
			}
